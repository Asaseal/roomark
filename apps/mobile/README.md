...
产品状态和软装项目读取最多等待 8 秒；产品状态超时会进入现有设备内置房源恢复结果，软装项目超时或读取失败则阻止空白项目进入编辑，并提供“重试读取 / 返回房源库”。迟到读取不会覆盖用户已经开始使用后的当前状态。写入不使用读取超时保护，避免不可取消的旧写入迟到覆盖新数据。

房源选择、比较、扫描和软装摘要写入失败后会继续显示现有重试提示；如果应用随后离开前台且当前没有写入进行中，Roomark 会自动重试一次最新内存快照，降低系统终止进程前丢失本次修改的风险。该重试不使用后台任务、定时循环或写入超时。
...
设备暂时无法读取某个房间的软装记录时，Roomark 不会把临时空白项目交给 3D 编辑器，也不会触发可能覆盖原布局的自动保存。后台预读取会暂停该房间，避免房源状态变化触发无提示的连续重试；进入软装页后可由用户明确选择重试，成功后恢复原有工作流，其他房间仍可正常使用。已读到但内容损坏的记录继续使用下方的逐项恢复逻辑。

软装写入队列、保存错误和保存中状态均按 `roomId` 隔离；同一房间的写入保持严格串行，一个房间永久等待或失败不会阻塞其他房间保存，也不会让错误状态串到其他房间。同一房间有多次排队保存时，最后一次结算前会持续显示保存中。退出后的异步保存失败会留在对应房间，重新进入后可用原有“重试保存”动作再次写入。
